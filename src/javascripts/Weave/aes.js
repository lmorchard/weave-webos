  ...
  return {
    encrypt: function(key, iv, data) {
      ...
      for (var i = 0; i < nOfBlocks; i++) {
        var ptext = data.slice(i * 16, (i + 1) * 16);
        var block = Weave.Util.XOR(cipher, ptext);
    ...
    decrypt: function(key, iv, data) {
      if (data.length % 16 != 0)
        return false;
      var rkey = Weave.Util.StA(key);  
      var rdat = Weave.Util.StA(data);
      
      AES_Init();
      AES_ExpandKey(rkey);
      
      var output = [];
      var cipher = Weave.Util.StA(iv);
      var nOfBlocks = data.length / 16;
      ...
      for (var i = 0; i < nOfBlocks; i++) {
        ...
        output = output.concat(Weave.Util.XOR(cipher, block, true));
